/**
 * Require an environment variable in production.
 * In development/test, fall back to the provided default so the server
 * starts without a full .env file.  In production, throw early so a
 * mis-configured deployment fails loudly instead of running with a
 * publicly-known secret.
 */
function requireEnv(key: string, devDefault: string): string {
  const value = process.env[key];
  if (!value) {
    if (process.env.NODE_ENV === 'production') {
      throw new Error(
        `[Config] Missing required environment variable: "${key}". ` +
        'Set it before starting the server in production.'
      );
    }
    return devDefault;
  }
  return value;
}

export const config = {
  PORT: Number(process.env.PORT) || 4001,
  NODE_ENV: process.env.NODE_ENV || 'development',
  LOG_LEVEL: process.env.LOG_LEVEL || 'info',
  MONGODB_URL: process.env.MONGODB_URL,
  UPSTASH_REDIS_REST_URL: process.env.UPSTASH_REDIS_REST_URL,
  UPSTASH_REDIS_REST_TOKEN: process.env.UPSTASH_REDIS_REST_TOKEN,

  OTP_TTL: Number(process.env.OTP_TTL) || 600, // 10 minutes
  OTP_RESEND_COOLDOWN: Number(process.env.OTP_RESEND_COOLDOWN) || 180, // 3 minutes
  OTP_RATE_MAX_PER_HOUR: Number(process.env.OTP_RATE_MAX_PER_HOUR) || 5,
  OTP_MAX_VERIFY_ATTEMPTS: Number(process.env.OTP_MAX_VERIFY_ATTEMPTS) || 5,
  OTP_HMAC_SECRET: requireEnv(
    'OTP_HMAC_SECRET',
    '09dc0abbb2961391d822610b31b912e3231d4d2745c76b1ef4765af4c62f6079',
  ),

  BREVO_API_KEY: process.env.BREVO_API_KEY || '',
  FROM_EMAIL: process.env.FROM_EMAIL || '',
  FROM_NAME: process.env.FROM_NAME || '',

  JWT_ACCESS_SECRET: requireEnv(
    'JWT_ACCESS_SECRET',
    '0f8bf908f8d38527c188c93bda49d48bd421a43fa0bdf3e77de1f0db785e6f37',
  ),
  JWT_REFRESH_SECRET: requireEnv(
    'JWT_REFRESH_SECRET',
    '826d2c0edb5ad8f8ac7668556c034ea228931a49576aefccc80d6f469cc4a34c4da82ca43a5c43de91ffdad2f4644c655e2eb3ccbb8bc2848cb64fe7ea2a1ab9',
  ),
  ACCESS_TOKEN_EXP: process.env.ACCESS_TOKEN_EXP || '15m',
  REFRESH_TOKEN_EXP: process.env.REFRESH_TOKEN_EXP || '7d',
  ACCESS_TOKEN_EXP_SEC: Number(process.env.ACCESS_TOKEN_EXP_SEC) || 900,
  REFRESH_TOKEN_EXP_SEC: Number(process.env.REFRESH_TOKEN_EXP_SEC) || 604800,
  REDIS_USER_TTL: Number(process.env.REDIS_USER_TTL) || 86400,

  // ── Bunny Storage (thumbnails, avatars) ────────────────────────────────────
  BUNNY_STORAGE_ZONE: process.env.BUNNY_STORAGE_ZONE || '',
  BUNNY_STORAGE_API_KEY: process.env.BUNNY_STORAGE_API_KEY || '',
  BUNNY_STORAGE_HOST: process.env.BUNNY_STORAGE_HOST || 'storage.bunnycdn.com',
  BUNNY_CDN_URL: process.env.BUNNY_CDN_URL || '',

  // ── Bunny Stream (lesson videos) ───────────────────────────────────────────
  BUNNY_STREAM_LIBRARY_ID: process.env.BUNNY_STREAM_LIBRARY_ID || '',
  BUNNY_STREAM_API_KEY: process.env.BUNNY_STREAM_API_KEY || '',
  BUNNY_STREAM_CDN_HOSTNAME: process.env.BUNNY_STREAM_CDN_HOSTNAME || '',
} as const;
